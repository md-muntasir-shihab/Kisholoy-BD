...
import { 
  ...
  Lock, LogOut, KeyRound, Building2
} from 'lucide-react';
...
export function AdminLayout() {
  ...
  return (
    <div id="admin-root-layout" className="min-h-screen bg-stone-100 flex flex-col font-sans">
      {/* Top Operational Header */}
      <header id="admin-top-header" className="sticky top-0 z-30 bg-stone-900 text-white border-b border-stone-800 h-16 flex items-center justify-between px-4 sm:px-6 shadow-xs">
        <div className="flex items-center gap-3">
          ...
          <Link to="/admin" className="flex items-center gap-2.5">
            ...
            <span className="text-teal-300 font-mono text-[10px] font-bold px-2 py-0.5 rounded bg-teal-950/80 border border-teal-800/80 uppercase tracking-wider">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-stone-800 hover:bg-stone-750 text-stone-200 hover:text-white border border-stone-700 text-xs font-semibold shadow-xs transition-colors"
          ...
          >
            ...
            <span className="hidden xl:inline text-stone-400 font-normal">| {isBn ? 'আমার অ্যাক্সেস' : 'Who Am I?'}</span>
          </button>

          {/* Quick Role Persona Switcher */}
          <div className="hidden sm:flex items-center gap-2 bg-stone-800 px-3 py-1.5 rounded-lg border border-stone-700">
          ...
          <button
            ...
            className="hidden md:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-stone-800 hover:bg-stone-750 text-stone-200 border border-stone-700 text-xs font-semibold shadow-xs hover:text-white transition-all"
          ...
          <button
            ...
            className="p-2 text-stone-300 hover:text-white rounded-lg hover:bg-stone-800 text-xs font-bold flex items-center gap-1 transition-colors"
          ...
          <Link
            ...
            to="/"
            target="_blank"
            className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-teal-900 hover:bg-teal-850 text-white text-xs font-semibold shadow-xs transition-colors"
          >
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside
          id="admin-sidebar"
          className={`fixed inset-y-0 left-0 z-20 w-72 bg-stone-900 text-stone-300 border-r border-stone-800 transform transition-transform duration-200 ease-in-out lg:translate-x-0 lg:static lg:inset-0 pt-16 lg:pt-0 flex flex-col ${
        ...
        >
          ...
          <div className="p-3 bg-stone-950/80 border-b border-stone-800 flex items-center justify-between">
            <button
              onClick={() => setInspectorOpen(true)}
              className="flex items-center gap-2 text-left w-full p-2 rounded-xl bg-stone-900 hover:bg-stone-850 border border-stone-800 text-stone-300 hover:text-white transition-all group"
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <span className="text-[10px] text-teal-400 block truncate">
                  Click to inspect access rules
                </span>
              </div>
              <KeyRound className="w-3.5 h-3.5 text-stone-500 group-hover:text-teal-400 transition-colors" />
            </button>
          </div>

          <nav id="admin-sidebar-nav" className="flex-1 overflow-y-auto p-3 space-y-5">
            {ADMIN_SECTIONS_DATA.map((section, sectionIdx) => {
              ...
              return (
                <div
                  ...
                  className={sectionIdx > 0 ? 'pt-4 border-t border-stone-800/70' : ''}
                >
                  {/* Section Header with Title */}
                  <div
                    id={`header-${section.id}`}
                    className="px-2 pb-1.5 flex items-center justify-between text-[11px] font-bold uppercase tracking-wider text-stone-400 select-none"
                  >
                    ...
                    <button
                      ...
                      className="p-1 text-stone-500 hover:text-teal-300 hover:bg-stone-800 rounded transition-colors"
                      ...
                      return (
                        <Link
                          ...
                          className={`flex items-center justify-between px-3 py-2 rounded-xl text-xs font-medium transition-all group ${
                            isActive
                              ? 'bg-teal-900 text-white font-semibold shadow-xs border border-teal-700/50'
                              : 'text-stone-400 hover:bg-stone-800/80 hover:text-stone-100'
                          }`}
                        >
                          <div className="flex items-center gap-2.5 truncate">
                            <Icon className={`w-4 h-4 shrink-0 ${isActive ? 'text-teal-300' : 'text-stone-500 group-hover:text-stone-300'}`} />
          ...
          {/* Sidebar Footer */}
          <div className="p-3.5 border-t border-stone-800 bg-stone-950/40 text-[11px] text-stone-400 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
              <span className="font-semibold text-stone-300">RBAC Active</span>
            </div>
            <button
              onClick={() => setInspectorOpen(true)}
              className="text-[11px] text-teal-400 hover:text-teal-300 font-semibold underline underline-offset-2"
            >
              Inspect Access
            </button>
          </div>
